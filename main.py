...
if 'df' not in st.session_state:
    st.session_state.df = pd.DataFrame(columns=['Name', 'Actionability', 'Feasibility', 'Business Value'])
...
if not st.session_state.df.empty:
    ...
    st.download_button("📥 Export as CSV", data=csv, file_name="ai_strategy_matrix.csv", mime='text/csv')
...
if not st.session_state.df.empty:
    ...
    # Add jitter
    plot_df['Feasibility_jitter'] = plot_df['Feasibility_num'] + np.random.uniform(-0.1, 0.1, size=len(plot_df))
    plot_df['Actionability_jitter'] = plot_df['Actionability_num'] + np.random.uniform(-0.1, 0.1, size=len(plot_df))
    ...
    fig.update_layout(
        ...
        yaxis=dict(title="Actionability", tickvals=[1, 2, 3], ticktext=['Low', 'Medium', 'High']),
